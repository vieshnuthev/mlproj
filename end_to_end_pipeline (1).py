...
# Use a non-interactive backend so plots can be saved reliably in one run.
matplotlib.use("Agg")
...
# Keep the dataset and outputs relative to the repo so the project is portable.
...
def sample_subset(dataset, sample_per_class: int, seed: int) -> torch.utils.data.Dataset:
    ...
    # Build a balanced subset by taking up to N images from each class.
...
def save_sample_grid(train_loader: DataLoader, class_names: list[str], show_plots: bool) -> None:
    ...
    def denormalize(image: torch.Tensor) -> np.ndarray:
        # Undo normalization so the saved preview looks like the original images.
        image = image * 0.5 + 0.5
...
def save_model_summary(model: nn.Module, show_plots: bool) -> None:
    ...
    with contextlib.redirect_stdout(buffer):
        # Capture the model printout into a text file for lecturer review.
...
def evaluate_model(
    ...
    # Shared validation helper used for both epoch-level checks and final scoring.
    model.eval()
    running_loss = 0.0
    correct = 0
    total = 0
...
def train_one_seed(
    ...
    for epoch in range(num_epochs):
        ...
        # Standard training step: forward pass, loss, backward pass, optimizer step.
...
def plot_training_curves(history: dict, seed: int, show_plots: bool) -> None:
    # Save the best-run learning curves so the report has a clean visual summary.
...
def plot_multi_seed_accuracy(results: list[dict], show_plots: bool) -> tuple[float, float]:
    # Compare stability across seeds instead of relying on a single lucky run.
...
def plot_best_run_artifacts(
    ...
    # Reuse the strongest run to generate the final report visuals.
...
def main() -> None:
    ...
    # Use the first seed for dataset sampling so every run works from the same subset.
